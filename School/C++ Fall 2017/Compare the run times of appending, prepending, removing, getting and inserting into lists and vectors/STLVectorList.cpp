...
int main()
{
	...
	outf << "Done! Total time: " << elapsed_time.count() << " seconds" << endl;
	cout << "Done!!" << endl;
	outf.close();
	return 0;
}
	...
	throw (string)
{
	...
	for (int i = 0; i < test_name.length(); i++) outs << "=";
	outs << endl << test_name << endl;
	for (int i = 0; i < test_name.length(); i++) outs << "=";
	outs << endl;
	outs << "          |---------------Vector-------------|";
	outs << "   |-----------------List-------------|" << endl;
	outs << "   Size        Time  Creates   Copies Destroys";
	outs << "        Time  Creates   Copies Destroys" << endl;
	...
	for (int size : SIZES)
	{
		outs << setw(7) << commafy(size);

		// Run and time the vector test and print its statistics.
		SortedVector sv;
		long etv = timed_test(sv, size, fv);
		outs << setw(9) << commafy(etv) << " ms";
		outs << setw(9) << commafy(Node::get_constructor_count());
		outs << setw(9) << commafy(Node::get_copy_count());
		outs << setw(9) << commafy(Node::get_destructor_count());
		outs.flush();
